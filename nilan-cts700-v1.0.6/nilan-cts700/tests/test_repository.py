...
def test_dashboard_has_distinct_images_for_every_control_state() -> None:
    """Every displayed state must resolve to a visibly distinct asset."""
    dashboard = yaml.safe_load((ROOT / "dashboard" / "nilan_panel.yaml").read_text())
    controls = [
        element
        for element in dashboard["elements"]
        ...
        in {
            "select.nilan_compact_p_nilan_ventilationstrin",
            "switch.nilan_air9_nilan_centralvarme_drift",
        }
    ]
    ...
    for control in controls:
        normal_images = [
            path
            for state, path in control["state_image"].items()
            if state not in {"unknown", "unavailable"}
        ]
        assert len(normal_images) == len(set(normal_images))


def test_write_only_controls_keep_the_last_successful_command() -> None:
    """Guard the optimistic state needed by affected CTS700/AIR9 firmware."""
    select_source = (COMPONENT / "select.py").read_text()
    switch_source = (COMPONENT / "switch.py").read_text()

    assert "self._commanded_option = option" in select_source
    assert "return self._commanded_option" in select_source
    assert "optimistic=True" in switch_source
    assert "self._optimistic_is_on = turn_on" in switch_source
